...
% Footer
% \setbeamertemplate{frame footer}{Máster en Bioinformática y Biología Computacional (2019-2020)}
...
\begin{frame}[fragile]{Papel del sistema inmune en la enfermedad}
  % \Fontvi
    \begin{itemize}
      \item En tumores sólidos establece lo que se conoce como contexto inmune: localización, densidad y organización funcional.
      \item Sisteme inmune presenta un papel dicotómico: 
      \begin{itemize}
        \item Supresión tumoral: linfocitos T CD8+, células B de memoria, etc.
        \item Progresión tumoral: linfocitos T reguladores, algunos macrófagos, etc. 
      \end{itemize}
      \item \alert{Terapias:} Quimioterapias inmunogénicas como oxaliplatino, anticuerpos inhibidores de puntos de control inmunitario: PD-1, CTLA-4, receptores coestimuladores de la respuesta inmumne, etc.
    \end{itemize}
  \vskip-1em
  \begin{figure}
    \centering
...
\begin{frame}[fragile]{Caso de estudio: cáncer de mama}
  ...
  \begin{columns}[t]
    ...
    \begin{column}{0.65\textwidth}
      \begin{alertblock}{Importancia del sistema inmune}
        \vskip-0.5em
          \begin{itemize}
            \item $\uparrow$ linfocitos T CD8+ $\rightarrow$ $\uparrow$ supervivencia.
            \item $\uparrow$ linfocitos T CD4+ reguladores $\rightarrow$ $\uparrow$ efecto inmunosupresor.
            \item $\uparrow$ TILs $\rightarrow$ beneficioso en pacientes tratados con trastuzumab.
            \item Quimioterapias neoadyuvantes.
          \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}{Objetivos}
  ...
  % \end{alertblock}
  \begin{enumerate}
    \item \alert<1>{Transformación de la \textit{pipeline} original en un paquete de R.}
          \begin{itemize}
          ...
          \end{itemize}
    \item \alert<2>{Análisis de datos \textit{scRNA-seq} procedentes de cáncer de mama.}
          \begin{itemize}
            \item Separación tipos tumorales y no tumorales.
            \item Identificación de tipos celulares no tumorales.
          \end{itemize}
    \item \alert<3>{Aplicación de la herramienta implementada.}
          \begin{itemize}
...
\begin{frame}[fragile]{Fundamento del método}
  \begin{overlayarea}{\linewidth}{1\textheight}
    ...
    \begin{enumerate}
      \alert<2>{\item Entrada de perfiles \textit{single-cell} caracterizados.} Simulación de nuevos perfiles con el modelo ZINB-WaVE en caso de tipos celulares poco representados o pocas células de partida.
...
\begin{frame}{digitalDLSorteR: paquete de R}
...
\begin{frame}[fragile]{2. Generación de la matriz de composición celular}
  ...
  \begin{textblock*}{\textwidth}(0.5cm,7cm)
    ...
    \begin{itemize}
      ...
      \item 5 métodos distintos para evitar posibles sesgos en la composición de las muestras \textit{bulk}.
...
\begin{frame}[fragile]{3. Simulación de perfiles \textit{bulk}}
  ...
  \begin{columns}
    ...
    \begin{column}{0.65\textwidth}
      \Fontvi
      \begin{itemize}
        \item Sumatorio de los niveles de expresión del gen $i$ de \texttt{n.cells} (100 por defecto) células ($z$) en función del tipo celular ($k$) al que pertenezcan para cada muestra \textit{bulk} ($j$).
...
\begin{frame}[fragile]{3. Preparación de los datos para entrenamiento y evaluación}
  ...
  \begin{itemize}
    ...
    \item Aleatorización de las muestras.
...
\begin{frame}[fragile]{4. Entrenamiento de la Red Neuronal Profunda}
  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      ...
      \Fontvi{
        \alert{\textbf{Instalación:}} README del repositorio: \href{https://github.com/diegommcc/digitalDLSorteR}{\url{diegommcc/digitalDLSorteR}}.
...
\section[Análisis de datos \textit{scRNA-seq} de cáncer de mama]{Análisis de datos \textit{scRNA-seq} de cáncer de mama}
...
\begin{frame}[fragile]{Análisis: separación de células tumorales y no tumorales}
  ...
  \begin{columns}
    \begin{column}{0.7\textwidth}
      \Fontvi{\alert{--} Aproximación seguida por los autores del artículo: comparativa de los \alert{patrones de expresión} entre células y tejido mamario normal.\\\alert{--} Cálculo de Z-scores y clusterización jerárquica (UPGMA).}
      \begin{figure}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        ...
        \item Clusterización: 10 clústeres.
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{Matrices de composición celular y modelos}
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \Fontvi
      \begin{alertblock}{Parámetros utilizados}
        \begin{itemize}
          \item 31.000 muestras \textit{bulk} compuestas por 100 células cada una. 
          \item Proporciones generadas por cada modelo: por defecto $\rightarrow$ 85\% con mayor aleatoriedad. 
        \end{itemize}
      \end{alertblock}
...
\begin{frame}{digitalDLSorter como método de deconvolución}
  \begin{alertblock}{Resultados obtenidos}
    \begin{itemize}
      \item Modelos entrenados con perfiles \textit{scRNA-seq} procedentes del entorno de estudio.
      \item Mejora constante de conjuntos de datos \textit{scRNA-seq} $\rightarrow$ mejores modelos de deconvolución.
      \item ZINB-WaVE ofrece buenos perfiles \textit{single-cell} simulados. 
      \item Uso de Redes Neuronales: buenas representaciones de los tipos celulares. 
      \item Simulación de perfiles \textit{bulk}: reduce el ruido intrínseco de los perfiles \textit{single-cell} y fuerza a encontrar patrones específicos. 
    \end{itemize}
  \end{alertblock}

  \begin{alertblock}{Limitaciones y trabajo futuro}
    \begin{itemize}
      \item Altamente dependiente de la calidad de los datos de partida. 
      \item Interpretabilidad pobre: naturaleza de 'caja negra' de las redes neuronales. 
    \end{itemize}
  \end{alertblock}
...
\begin{frame}{digitalDLSorteR como paquete de R}
  ...
  \begin{alertblock}{Aportaciones}
    \begin{itemize}
      \item Implementación formal de cada uno de los pasos. 
      \item Funciones de evaluación del desempeño de los modelos. 
      \item Parámetros que permiten modelos más personalizables. 
      \item Uso de ficheros HDF5 como \textit{back-end}. 
    \end{itemize}
  \end{alertblock}
% Decir que con los perfiles \textit{single-cell} no basta
  \begin{alertblock}{Limitaciones y trabajo futuro}
    \begin{itemize}
      \item Ausencia del directorio test en el paquete: han sido incluidos los test correspondientes a la carga de datos (fichero loadData.R) y a la generación de las matrices de composición celular (fichero generateTranAndTestProbMatrix.R).
      \item Carga de las muestras \textit{bulk} simuladas en memoria antes de su escritura en disco como ficheros HDF5. 
      \item Imposibilidad de paralelizar entrenamiento red neuronal.
      \item Permitir el uso de redes neuronales arquitecturas diferentes. 
      \item Implementación del método como servicio Web (aplicación Shiny).
    \end{itemize}
  \end{alertblock}
...
\begin{frame}{Referencias}
  ...
  \hskip-1em
  \begin{itemize}
    \item W. Chung, H. H. Eum, H. O. Lee, K. M. Lee, H. B. Lee, K. T. Kim, H. S. Ryu, S. Kim, J. E.
    Lee, Y. H. Park, Z. Kan, W. Han, y W. Y. Park. Single-cell RNA-seq enables comprehensive
    tumour and immune cell profiling in primary breast cancer. Nat Commun, 8:15081, may.
    2017. doi: 10.1038/ncomms15081.
    \item C. Torroja y F. Sanchez-Cabo. Digitaldlsorter: Deep-Learning on scRNA-Seq to deconvolute
    gene expression data. Front Genet, 10:978, oct. 2019. doi: 10.3389/fgene.2019.00978.
  \end{itemize}
\end{frame}
